...
void update_table_evicted_frame(struct queue *q, int evicted_frame, struct PCB *current_process) {
    if (current_process) {
        for (int page = 0; page < current_process->num_pages; page++) {
            if (current_process->page_table[page] == evicted_frame) {
                current_process->page_table[page] = -1;
            }
        }
    }
...
void page_faults_handler(struct PCB *pcb, struct queue *ready_queue) {
    ...
    if (frame == -1) {
        ...
        update_table_evicted_frame(ready_queue, victim, pcb);
...
int frame_store_is_full() {
    return next_free_frame >= FRAME_STORE_SIZE / PAGE_SIZE;
}