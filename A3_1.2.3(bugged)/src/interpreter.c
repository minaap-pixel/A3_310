...
struct PCB *run_pcb_to_completion(struct PCB *pcb) {
    while (pcb_has_next_instruction(pcb)) {
        ...
        if (!instr) break; // stop running the process if no instruction
        parseInput(instr);
    }
    free_pcb(pcb);
    return NULL;
...
struct PCB *run_pcb_for_n_steps(struct PCB *pcb, size_t n) {
    ...
    for (; n && pcb_has_next_instruction(pcb); --n) {
        const char *instr = pcb_next_instruction(pcb);
        if (!instr) break; // stop running the process if no instruction
        parseInput(instr);
    }
...
struct queue *q = NULL;
const struct schedule_policy *policy = NULL;
    ...
    for (int n = 0; n < args_size; ++n) {
    	...
    	if (existing) {
        	...
        	*pcb = *existing;           // copy all fields including page_table
        	pcb->pc_page = 0;          // but reset the program counter
            pcb->pc_offset = 0;
        	pcb->next = NULL;
